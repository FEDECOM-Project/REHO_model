...
class district_decomposition:
    ...
    def __init__(self, qbuildings_data, units, grids, parameters=None, set_indexed=None,
        ...
        if self.method["read_electricity_profiles"] is not None:
            #tempo=el_parser.read_typical_profiles(self.method["read_electricity_profiles"], self.File_ID)
    ...
    def SP_initiation_execution(self, scenario, Scn_ID=0, Pareto_ID=1, h=None, epsilon_init=None, beta=None):
        ...
        if self.method['fix_units']: # ici fix unit district_decomp
            for unit in self.fix_units_list:
                if unit == 'PV':
                    print('fix_PV_SP_execution')
                    print(ampl.getVariable('Units_Mult').get('PV_' + h))
                ...
                elif unit == 'Battery_district':
                    print('fix_batt_SP_execution')
                    #print(ampl.getVariable('Units_Mult').get(unit))
    ...
    def SP_execution(self, scenario, Scn_ID, Pareto_ID, House):
        ...
        if self.method['fix_units']: # ici fix unit district_decomp
            for unit in self.fix_units_list:
                if unit == 'PV':
                    print('fix_PV_SP_execution')
                    print(ampl.getVariable('Units_Mult').get('PV_' + House))
