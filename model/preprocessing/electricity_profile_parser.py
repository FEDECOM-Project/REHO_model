...
def read_typical_profiles(typical_file='typical_profiles_lugaggia.csv', File_ID='Lugaggia_10_24_T_I'):

    df_E = pd.read_csv(os.path.join(path_to_electricity, typical_file), index_col=[0]) # remarque : le path attribué à "typical_file" semble ne pas s'implémenter
    df_E = df_E/1000  # from  W to kW
    df_E.columns = df_E.columns.astype(int)  # change column name from string to int
    df_E=pd.read_csv(os.path.join(path_to_electricity, 'typical_profiles_lugaggia.csv'), index_col=[0])
    ...
    for p in df.index:
        ...
        print(difference)
        dif_h = int(difference.total_seconds() / 3600) #dif hours in hours
        print(dif_h)
