...
def df_sankey(df_results, label='FR_long', color='ColorPastel', precision=2, units='MWh', display_label_value=True):
    ...
    elec_storage_list = ['Battery', 'EV_district', 'Battery_district']
    ...
    semi_auto_device  = ['HeatPump_Air', 'HeatPump_DHN', 'NG_Boiler', 'ThermalSolar', 'OIL_Boiler',
                         ...
                         'HeatPump_Lake', 'WOOD_Stove', 'HeatPump_Geothermal', 'Air_Conditioner_Air','Air_Conditioner_DHN',
                         'DataHeat_DHW','NG_Boiler_district'] # name must be the same as used by REHO
...
def plot_sankey(source, target, value, label_, color_,sc):
    ...
    fig.update_layout(title_text=sc, font_size=10)
    fig.show()